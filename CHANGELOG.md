...
- **Combat actions (attack, dodge, aim, burst, condition, willpower, reload, throw_grenade, etc.) intentionally remain web-UI-only** — decision unchanged from v0.15.7. The MCP surface is for GM prep and lifecycle, not in-fight resolution. The GM-side LLM should not be making to-hit rolls
- `get_encounter` payload expanded — adds `is_hidden`, `metadata.is_surprise_round`, `story_idea_title` (was already exposing `story_idea_id`), plus per-participant `offhand_weapon_name`, `offhand_weapon_data`, `is_alert` (alias for `surprise_immune`), `is_ready` (derived from condition tag), `aim_state` (`{target_id, turns}` or `null`), `ammo_state`, `offhand_ammo_state`, `grenade_inventory` (slug → count map), `wound_penalty`, `attack_modifier_total` (wound + condition modifier sum). Existing fields untouched
- **Equip via MCP bypasses the v0.15.23 turn-cost rule** — treated as `instant_equip=True` always. MCP equips are GM prep, not in-fight equip swaps; the `acted_this_round` flag is never set by the API. Logged with `action_cost: "instant"`
- Backed by 9 new JSON API endpoints under `/api/admin/combat/`: `visibility/`, `initiative/clear/`, `participants/<id>/initiative/`, `participants/<id>/equip/`, `participants/<id>/cover/`, `participants/<id>/grenades/`, `participants/<id>/adjust-hp/`, `participants/<id>/alert/`. All under `_api_gm_only` Bearer-token auth, all CSRF-exempt POST + `transaction.atomic`, all writing standard `_log` rows so the timeline reflects MCP-driven changes the same as web-UI ones
- exodus-mcp repo is local-only (no remote configured); no Docker rebuild needed for the MCP side. The Django repo deploys via the standard `elohite/exodus:0.15.36` image flow
- No model schema changes, no migrations, no new dependencies. All 70 v0.15.35 combat tests still pass

## v0.15.35
- **Combat unit tests — first-pass infrastructure.** Added `combat/tests/` package with `__init__.py`, `factories.py` (no-dep test data helpers — `make_user` / `make_character` / `make_npc` / `make_encounter` / `make_participant` / `make_merit` / `attach_merit`), `test_helpers.py` (pure-helper coverage of `_roll_pool`, `_apply_damage`, `_wound_penalty`, `_distribute_gun_fu`, `_parse_weapon_damage`, `_clamp_again_local`, condition-tag parsers, `_normalize_dice_payload`, condition modifiers), and `test_views.py` (integration coverage of encounter CRUD + visibility gates, initiative lifecycle, attack resolution branches — ammo / cover / surprise / alert immunity, GM-or-owner permission gates, player-ready gate at start, GM burn tick, knockdown auto-trigger, grenade inventory + EMP immunity + scatter)
- **70 tests, ~10 second suite duration on `manage.py test combat`.** All tests pass against the current v0.15.34 codebase. The wound-penalty merit overrides (Pain Tolerance / Increased Pain Threshold), Gun Fu auto-success distribution, WoD 2.0 damage track upgrade ladder (B → L → A), and the v0.15.32 per-round initiative re-roll all carry pinned regression coverage now
- **No production code changes, no new dependencies, no schema changes.** Pure Django `TestCase` / `SimpleTestCase`. Future refactors of `combat/views.py` will catch regressions before deploy — the test suite documents expected behaviour as much as it verifies it
- Future work: WebSocket consumer auth tests (would require `pytest-asyncio` / `channels.testing.WebsocketCommunicator`); deferred to keep v0.15.35 dependency-clean. Documented as a `TODO` comment in `combat/tests/test_views.py`

## v0.15.34
- **EXTINGUISH action** — players can now drop the `burning` condition by spending their turn on the action (drop and roll, dunk in water, smother). The GM can fire the action off-turn for free narrative bookkeeping (an ally pats out the fire). New `combat:extinguish` URL + view + per-row EXTINGUISH button visible only when the row carries the `burning` tag. Logged as `extinguish` with `on_turn` payload key. Closes the v0.15.30 deferred item — burning previously cleared only via × on the BURNING pill
- **POSITION (narrative movement)** — gridless three-slot model: `engaged` (melee), `short` (5–10 m), `long` (20 m+). Per-row POSITION sub-form with three buttons (ENGAGED / SHORT / LONG); player moves cost the turn, GM moves are free narrative repositioning. New `combat:move_position` URL + view. Position label gates the new REACH bonus (both combatants must be at engaged for reach to fire). Default position is `engaged` for every participant — no migration needed because the field already existed since v0.15.0
- **DISARM action** — single-target WoD 2.0 contest. Attacker rolls Dexterity + Brawl (or Weaponry, override) + GM modifier + wound + condition mods; target rolls Dexterity + Brawl (clean resistance, no wound penalty). Attacker successes strictly greater than target ⇒ target's main-hand weapon is unequipped (snapshot cleared, ammo tag stripped). Tie or miss ⇒ target keeps the weapon. New `combat:disarm` URL + view + collapsible DISARM ATTEMPT sub-form on the active attacker's row. Logged as `disarm` with both pools, both success counts, both dice arrays, and `outcome ∈ {disarmed, kept}`
- **COUP DE GRÂCE action** — finishing blow on an `incapacitated` target. **No roll**: auto-applies 3 aggravated damage via `_apply_damage` (track-upgrade ladder honoured). New `combat:coup_de_grace` URL + view + per-row form, only visible when at least one OTHER participant in the encounter is incapacitated. Logged as `coup_de_grace` with before/after track snapshots and applied/upgrades/overflow split
- **Cover destruction** — each landed attack against a target in cover consumes 1 point of `cover_health`. When health hits 0 the cover collapses (cover_state cleared, log row written). Cover damage is applied BEFORE the participant damage so a final bullet that destroys the cover ALSO hits the body. The COVER pill on the row reads `HP N` when active and clears entirely on collapse. New `cover_destroyed` log row carries the cover entry name; attack payloads gain `cover_damage_taken` + `cover_destroyed` keys
- **REACH (melee)** — long melee weapons get +1 dice in melee against shorter weapons when both combatants are at `position_label="engaged"`. Capped at +2 when the difference is large. New `reach: int` field on the weapon catalogue (default 0; clamped 0..5 via `_clamp_reach`). Default seed: Knuckle Buster / Knife / Phone Book / Bottle / Taser at 0; Baton / Chair / Hammer at 1; firearms / thrown / grenades all 0. Pool breakdown surfaces as `(+N REACH)`. Off-hand consults its own weapon's reach independently
- **Grenade scatter on miss** — a grenade throw that rolls 0 successes scatters off-target. The grenade is still consumed from inventory and the thrower's turn is still spent; the timeline gets a `GRENADE SCATTERED` system warning + a `grenade_scatter` log row carrying the original target list. GM picks the new target list narratively (or rules no effect) — gridless model means the resolver can't project a scatter direction
- **NPC merit_uses for Gun Fu** — `_gun_fu_state` now also resolves NPC actors (the merit through-table `npc_merits` and the new `NPC.merit_uses` JSONField). GM-driven NPCs with the Gun Fu merit can now spend the auto-success pool the same way a player would on a Character. Spend persistence writes back to `NPC.merit_uses["Gun Fu"]`; reset via the same admin / API surface as the rest of the NPC sheet. Migration `npcs/0010_npc_merit_uses` adds the field with `default=dict` so existing NPC rows survive the upgrade
- **Throwing range bands (medium)** — `_parse_weapon_damage` now recognizes a third `medium` band on free-text damage strings (e.g. `"2L close / 2L medium / 1L long"`). Resolver / preview / attack form honour the band when the catalogue entry defines one. Seed catalogue unchanged — thrown weapons still ship single-band; the GM can opt in per-weapon. Single-band fallback semantics from v0.15.26 fully preserved
- Settings UI weapons editor gains a REACH column on every category (parallel-array `weapons_<cat>_reach`); JS row-add inserts the cell with default 0
- Rules explainer at `/rules/combat/` gains REACH (MELEE), POSITION (NARRATIVE MOVEMENT), EXTINGUISH, DISARM, COUP DE GRÂCE, COVER DESTRUCTION subsections; GRENADES gains a SCATTER ON MISS bullet; BURNING bullet now points at EXTINGUISH instead of × on the pill
- One migration: `npcs/0010_npc_merit_uses`. No other schema changes, no new dependencies
