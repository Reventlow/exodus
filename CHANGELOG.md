...
## v0.12.45
- Seed Star Systems: one-click generation of resources, planets, and civilisations
- 6-tier scarcity controls: Very Common → Extremely Rare for each category
- Planet generation weighted by spectral class, atmosphere by planet type
- Life classification weighted (bacterial common → intelligent extremely rare)
- Full overwrite mode — re-seed anytime with different scarcity settings

## v0.12.44
- Planets of interest management in Settings > Star Map
- Select star system from dropdown, view/add/delete planets
- Full planet creation form: name, type, atmosphere, life, temperature, gravity, water, habitability
- Scan level required for discovery, hidden/visible toggle

## v0.12.43
- Planets of interest: add planets to star systems with atmosphere, life type, habitability
- Atmosphere types: oxygen, methane, argon, nitrogen, CO2, hydrogen, ammonia, toxic
- Life classification: prebiotic, bacterial, cellular, plant, animal, intelligent
- Planet discovery through scanning (scan level required)
- Full CRUD API for planets

## v0.12.42
- Star Map config in settings: fully custom resource types (add/remove/colour/icon)
- Civilisation system: Kardashev-scale tech levels (Type 0-III), dispositions, GM visibility
- Civilisations can be assigned to star systems with discovery mechanics
- Scan-to-discover: civilisations revealed when agency reaches required scan level

## v0.12.41
- Settings page redesigned: tab-based sidebar with 4 groups (Session, Interface, Maps, Admin)
- One section visible at a time, URL hash routing for deep linking
- Transfer Player in danger zone with red warning styling
- Mobile: sidebar collapses to horizontal tabs

## v0.12.39
- All nav bar labels customisable from settings (DISPATCH, PLAYERS, AGENCIES, COUNCIL, NPC'S, COMMS)
- Council can be enabled/disabled from settings
- Council mode: agency-based or player/NPC-based (setting stored, implementation ready)

## v0.12.37
- Player and NPC markers on city maps use character/NPC portrait images
- Right-click picker: select from character list (player type) or NPC list (npc type)
- Portrait shown as circular marker with pulsing ring
- NPC markers with portraits also get the portrait treatment

## v0.12.36
- Player marker type on city maps with pulsing violet indicator
- GM can drag player markers to update their position in real-time
- Position auto-saves to database on drag end
- Updated controls hint with drag instructions and player colour

## v0.12.34
- City maps: search any city, auto-geocode, dark Leaflet map with CARTO tiles
- GM can place markers (base/mission/npc/custom) via right-click
- Per-marker visibility control for players
- Settings page: search to add cities, toggle enabled/visible, delete
- City maps appear in nav bar when enabled
- Marker popups support markdown

## v0.12.33
- MAP renamed to WORLD MAP, placed next to STAR MAP in nav
- Map visibility toggles in settings page: WORLD MAP and STAR MAP can be shown/hidden for players
- Staff always sees both maps regardless of settings

## v0.12.32
- Nav bar redesigned: two-row command interface layout
- Command row: brand, session date, COMMS (with badge), PROFILE, LOGOUT
- Subsystem row: all navigation links grouped with pipe dividers
- Cleaner visual hierarchy, less clutter

## v0.12.30
- New starmap Django app: StarSystem, AgencyScan, ScanRollLog models
- Star catalog now database-driven with API import
- Per-agency scanning with exclusive, tradeable intel
- Scan dice pool from observatory + computer core + character stats
- Agency claims on star systems (first-to-arrive, requires scan)
- Scan data sharing between agencies
- Star map frontend loads from API with fallback to hardcoded catalog
- Endgame beacon: Signal Source Omega at 150 ly with pulsing violet glow

## v0.12.29
- Displacement Drive system on star map: v1 (1 jump) through v5 (10 jumps)
- Route planning: click stars to plot multi-jump routes with colored difficulty lines
- Navigation difficulty: Short/Medium/Long/Extreme based on leg distance
- Drive capacity check: shows FEASIBLE or EXCEEDS DRIVE CAPACITY

## v0.12.28
- Star map expanded from 26 to 63 real star systems
- Added brown dwarf (L/T/Y) and white dwarf (D) spectral colors
- New systems include TRAPPIST-1, Teegarden's Star, Wolf 1061, Gliese 581, and more
- Coverage now extends from 0-51 light years

## v0.12.27
- Transfer Player to Agency: converts character to NPC dossier, moves NPCs, optionally transfers XP and merits
- Available on settings page with confirmation dialog
- Cleans up workspace assignments on transfer

## v0.12.26
- Fix: cyber terminal defense is now per-user per-thread instead of shared across all members
- Each member has their own defense bonus and active state
- Passive detection uses the defender's own defense bonus
- GM clear_all resets all members' defense

## v0.12.25
- Eyes-only dispatches can now be shared with specific characters via "SHARE WITH" button
- Sharing is permanent — cannot be revoked once shared
- Shared recipients see the dispatch alongside original eyes-only badge + SHARED badges
- "RELEASE TO ALL" still available to make fully public

## v0.12.23
- Fix: impersonation banner and return-to-GM now works reliably
- Fallback: stop-impersonate URL always returns to superuser even without session key

## v0.12.20
- GM can impersonate any player from the settings page to see their view
- Warning banner with "RETURN TO GM" button shown on all pages while impersonating

## v0.12.19
- GM-only predicted votes on council items — set expected agency positions before sessions
- Clickable vote prediction grid per agency (for/against/abstain/undecided)
- MCP tool: update_council_item supports predictedVotes field

## v0.12.18
- Council actions restricted to Fixer class: voting, proposals, chairman actions
- Non-fixer players have read-only access to council page
- Both frontend buttons and backend API enforce the restriction

## v0.12.17
- GM manual completion changes are now logged in the roll log for both regular and FTL projects

## v0.12.16
- Fix: roll log button now works on regular projects (was using wrong agency ID variable)

## v0.12.14
- Fix: roll logging migration for live database — adds missing columns

## v0.12.13
- Roll logging now works for both regular projects and FTL projects
- Roll log button added to regular project cards

## v0.12.12
- Fix: agency page crash from roll log modal JSX fragment issue

## v0.12.11
- Project rolls are now logged with full dice results, successes, and progress
- Roll log button on each project opens a modal showing all past rolls
- Shows character name, dice, successes, auto-successes, progress, and timestamp

## v0.12.10
- Gain Access in cyber terminal now requires the target to have sent a message in the thread
- Removed UIC charter from settings page

## v0.12.9
- Comms and cyber terminal can be locked/unlocked from site settings
- Locked by default between sessions — only superusers can send messages or use cyber terminal
- Thread titles can now be updated by superusers via PUT

## v0.12.7
- Released eyes-only dispatches now show both EYES ONLY and RELEASED badges
- Original recipient visible to all players after release

## v0.12.6
- Full image mode on detail page shows image at natural height

## v0.12.5
- Full image mode now keeps same height as cropped mode with contain fit

## v0.12.4
- Featured images can now toggle between cropped and full display
- Superuser checkbox on article detail page to show full image without cropping

## v0.12.3
- Eyes-only badges now display character names instead of player usernames

## v0.12.2
- Dispatches now sorted by in-game date (newest first)
- Added sortable game date field for chronological ordering

## v0.12.1
- Dispatch articles now display in-game date instead of real-world timestamp
- In-game date field added to article editor
- Fixed PDF attachment upload and linking

## v0.12.0
- New: Bifrost Dispatch — internal update panel as landing page
- Articles support markdown content, featured images, and file attachments
- Visibility levels: hidden (superuser only), eyes only (specific player), public (all)
- Eyes-only articles can be released to all players by the designated recipient
- Superuser article editor with markdown preview, image upload, and attachment management
- Operatives page moved to /characters/

## v0.11.50
- Base identity affects thrive — location type creates specialisation trade-offs:
  - Military Base / Black Site: -1 Science Ops per science facility, -1 Diplomatic Corps per diplomatic facility
  - R&D Installation / Observitorium: +1 Science Ops, +1 Engineering Ops baseline, -1 Military per military facility
  - Other locations: neutral, no modifiers
- New location type: R&D Installation (10 EXP, 20 space, general class)
- Collapsed base card header redesigned: two-row layout with department thrive pills
- Rules panel updated with Base Identity section
- Balance: barracks +2, admin gets sources from general/living/recreation/storage

## v0.11.48
- Balance: barracks now +2 military per level (was +1) — soldiers no longer disadvantaged vs other classes
- Balance: general areas +1 admin, living +1 admin, recreation +2 admin, storage +1 admin — admin no longer permanently Collapsing
- Security equipment affects thrive: internal security +1 admin, segmented +1 intel, high monitoring +1 intel/-1 science/-1 engineering
- All thrive bonuses are whole numbers (no more 0.5 values)
- Intelligence department moved to fixer class
- Exotic Materials Vault renamed (was Anomaly Vault)
- Base scroll no longer re-triggers on poll
- Rules panel updated with full thrive contribution details

## v0.11.42
- Child prodigy reworked: +2 automatic successes on every roll for their assigned project (on top of normal NPC dice bonus)
- Dedicated to one project until completion — cannot help other projects
- Removed child prodigy thrive penalty (integrity risk + dedication lock is enough cost)
- Active fringe projects still give -1 to all departments per project on that base
- Prodigy auto-successes shown in roll results alongside merit auto-successes

## v0.11.38
- RULES button on bases section — collapsible panel explaining class restrictions, power system, department thrive, global penalties, and special locks
- Underground bases now connected to country grid (only underwater/mobile/orbital are isolated)
- Department thrive tooltip shows full math breakdown on hover

## v0.11.35
- Department thrive scores now auto-calculated from facilities (no manual setting)
  - Each facility contributes to relevant departments (some with cross-department effects)
  - Interrogation boosts Intelligence but tanks Diplomatic Corps
  - Anomaly Vault boosts Science but unsettles Military
  - Medical and Hydroponics boost both their department and general morale
- Global modifiers affect ALL departments:
  - No housing on 5+ facility base: -1 (or -2 on 11+)
  - Cramped housing (L1 only): -1 | Luxury (L3): +1
  - No medical on 8+ facility base: -1
  - Poor amenities (<10% general/recreation): -1 | Good (20%+): +1
  - No power on isolated (underground/underwater/mobile) base: -1
- Collapsed base card shows thrive scores color-coded
- Existing base departments reset for auto-calculation

## v0.11.28
- Power grid toggle system:
  - Each facility shows ⚡N power cost (yellow) or ⚡+N output (green for power plants)
  - Clickable ⚡ toggle next to each active facility — connects/disconnects from internal power
  - Engineers and admins can toggle; other classes see read-only
  - Mobile/underground/underwater bases: all facilities forced to internal power (no toggle)
  - Power bar shows internal consumption vs production + grid dependency percentage
  - Workspaces also have power cost display and toggle
- Solar/Wind locked on underground/underwater bases
- Geothermal locked on surface/mobile bases

## v0.11.24
- Power system for bases:
  - All facilities now have power consumption values
  - Power plants produce power: Generator (15), Solar/Wind (30), Geothermal (40), Nuclear Reactor (80)
  - Power bar shows consumption vs production on each base
  - Non-mobile bases can exceed power budget (draws from country grid, shown as warning)
  - Mobile/underground/underwater/orbital bases must be self-powered (deficit shown as error)
  - Power plants stack: Generator + Solar = 45 total
- Nuclear power status: superuser checkbox with ☢ icon on agency header
  - Nuclear Reactor facility locked unless agency has nuclear power status
- Geothermal Plant added (EXP 8, Size 5, Output 40) — ideal for underground/underwater bases
- Nuclear Reactor renamed, size increased to 8

## v0.11.23
- Server-side validation for player base building:
  - XP check: total base EXP across all bases cannot exceed agency experience
  - Space check: facilities/workspaces can't exceed base capacity
  - Space-adding items (location types, size merits) are never blocked by space
  - Admin bypasses all checks

## v0.11.22
- Fix: re-applied required_class on 15 facilities and 5 equipment items in production DB (earlier curl fix was overwritten by container restart)
- Removed debug fields from API response

## v0.11.20
- Fix: other-class facilities no longer leak into player view — full definitions moved to separate lookup field (baseConfigLookup), completely isolated from buildable options (baseConfig)

## v0.11.19
- Shipyard reclassified to soldier (was engineer)
- Location type: once chosen, other options hidden for non-admin players

## v0.11.18
- Fix: other-class built items now show proper names and levels (e.g. "Airstrip" not "Level 1")
  - Full facility/location/merit/equipment definitions sent for display lookups
  - Cost calculations also use full defs so EXP totals are correct for all classes

## v0.11.17
- Engineers can add workspaces and assign characters/NPCs to them (not just admins)
- Remove workspace button remains admin-only
- 16 facilities reclassified on production: shipyard→engineer, motorize/drone/interrogation/training→soldier, lab/observatory/anomaly vault/hydroponics→science, HR/diplomatic/intel archive/safe room→fixer, etc.

## v0.11.16
- Base building permissions reworked for collaborative play:
  - Players see built items from all classes (read-only for other classes)
  - Players see unbuilt options only for their own class
  - Players can ADD items from their class but cannot REMOVE anything
  - Only superusers can remove items, clear bases, or delete bases
  - Server-side additive-only validation for non-admin PUT requests

## v0.11.15
- New facility: Fringe Science Lab (science class, 15 EXP, 5 space)
  - Locked behind "Fringe Science Labs" pulling string — shows padlock if nobody has it
  - Projects can only be marked as fringe if their assigned base has a Fringe Lab
  - Fringe hand icon disabled with tooltip when base lacks the lab

## v0.11.14
- Project deletion now requires confirmation dialog

## v0.11.13
- Superuser CLEAR BASE button: wipes all choices (location, merits, facilities, workspaces, equipment) with confirmation showing EXP returned. Keeps name, departments, and notes.
- DELETE BASE now also has a confirmation dialog

## v0.11.12
- Players can manually spend auto-success merit uses from the character sheet (USE button next to session dots)

## v0.11.11
- Players can now see the reroll threshold on projects (shows green "REROLL 8+" or "REROLL 9+" when set below 10)

## v0.11.10
- Fix: mental load dice pool penalty now uses the correct per-character rules
  - Total boxes = 4 + floor((Composure + Resolve) / 2) — varies per character
  - Only the last 4 boxes give penalties, and only cognitive penalties affect project rolls
  - Box 1: -2 social only (no project impact), Box 2: -2 cognitive, Box 3: -1 cognitive, Box 4: -2 cognitive

## v0.11.9
- Fix: mental load penalty simplified to last 4 boxes

## v0.11.8
- Fix: completion score field now updates after rolling (key-based re-mount)

## v0.11.7
- Roll results now display inline under the roll button (no more alert dialogs)
  - Dice shown as styled boxes: green for successes, grey for misses
  - Shows successes, auto-success bonus, completion progress, mental load
  - Works on both project and FTL rolls

## v0.11.6
- Auto-success merits now use clickable toggle buttons instead of prompt()
  - Click the merit button to arm it (shows checkmark + highlights)
  - Roll button updates to show armed merit name
  - Confirm dialog shows the auto-success bonus
  - Works on both projects and FTL rolls

## v0.11.5
- Fix: auto-success merit dots not showing on project roll button (wrong prop name)

## v0.11.4
- Fix: character sheet blank page — IS_ADMIN → IS_SUPERUSER (correct variable name for character sheet)

## v0.11.3
- Fix: character sheet Babel standalone compatibility for merit usage dots

## v0.11.2
- Auto-success merits: I Know a Guy (fixer), I Have a Theory (science), McGyver It (engineer), Gun Fu (soldier)
  - Adds automatic successes equal to merit rating (not dice — guaranteed successes)
  - Usable rating times per session (e.g. rating 3 = 3 uses of +3 auto-successes)
  - One merit per roll — player chooses at roll time via prompt
  - Usage tracking dots on character sheet and below project/FTL roll buttons
  - GM "RESET USES" button on character sheet to clear at session start
- Base department thrive scores now affect project dice pools

## v0.11.1
- Base building options filtered by character class — no more quarterbacking
  - Soldier: military base, black site, armored, barracks, armory, brig, aviation, all defenses & aviation equipment
  - Engineer: underwater, underground, computer core, workspace, engineering project sites
  - Science: medical, science research project sites
  - Fixer: estate, front, auditorium, HR off-boarding, criminal org
  - General (all): official building, extra/super large, storage, internal security
- Department thrive scores on bases (1-10 scale)
  - Superuser selects which departments exist: Military, Intelligence, Engineering Ops, Science Ops, Diplomatic Corps, Admin, Criminal Org, Mercenaries
  - Thrive score provides dice pool modifier: 1-2 = -2, 3-4 = -1, 5-6 = 0, 7-8 = +1, 9-10 = +2
  - Departments linked to character classes — modifier auto-applied to project rolls
  - Flavor labels: Collapsing → Critical → Struggling → Strained → Stable → Functional → Thriving → Flourishing → Exemplary → Pinnacle

## v0.11.0
- Base building update (coming)

## v0.10.54
- Fringe rules panel updated with PERMANENT and PER ROLL badges on each effect
- Legend at top explains the two categories
- Footer updated to reflect per-roll re-activation mechanic

## v0.10.53
- Fringe effects split into permanent and per-roll:
  - Permanent (stay forever): Dark Grants, Live Testing, Black Market Tech, Gene Manipulation, Overclocked Equipment, Child Prodigy
  - Per-roll (consumed after rolling): Stimulants, Neural Interface, Sleep Deprivation
- Per-roll effects cleared after each completion roll, can be re-activated for the next roll
- Per-roll effects labeled "(next roll)" in dice pool breakdown with distinct pink color
- Personal NPC pulling strings removed from dice pool config (NPCs handled via ASSIGNED NPCs)
- NPC assignment shows only assigned NPCs with add/remove buttons (not a multi-select)
- NPCs filtered to project owner's personal NPCs only

## v0.10.48
- Personal NPCs assigned to Bifrost Union (agency=1) for all player-owned NPCs
- GM can now set agency on any NPC (not just dossiers/non-player agencies)
- NPC assignment dropdown includes personal NPCs via agency membership
- Downtime actions: Rest (-2/-1 ML), Study (+4/+2 XP), NPC Study (+5 XP)
- Reroll threshold: GM sets 8+/9+/10 on projects and FTL projects

## v0.10.47
- Reroll threshold: GM can set 8+, 9+, or 10 (default) on projects and FTL projects
  - 8+: dice showing 8, 9, 10 explode (reroll and add)
  - 9+: dice showing 9, 10 explode
  - 10: only 10s explode (standard)
- Shows green when set below 10

## v0.10.46
- Downtime actions in YOUR PROJECT ROLLS section:
  - REST: Free roll removes 2 ML, spare time removes 1 ML (no extra ML tax)
  - STUDY: Free roll gives +4 XP, spare time gives +2 XP (+1 ML)
  - NPC STUDY: Assign study to a personal NPC, +5 XP either way (spare time costs +1 ML)
- Agency page polls every 30 seconds for live updates

## v0.10.45
- Roll buttons show grayed out with "NO ROLLS (0 remaining)" when at 0 instead of hiding
- Player roll counts in Core Stats grayed out at 0
- Both regular projects and FTL projects updated

## v0.10.44
- Split _core_modules.html (2346 lines) into _core_modules.html (963) + _projects.html (1392)
- DarkGrantsModule and ProjectsModule moved to _projects.html

## v0.10.43
- FTL projects: full dice pool config UI (attributes, skills, merits, pulling strings)
- FTL projects: dice pool hover tooltip with color-coded breakdown
- Fix: FTL roll now uses computed dice pool

## v0.10.42
- FTL projects now have the full project system:
  - Player and base assignment
  - Fringe science toggle with Dark Grants, Black Market Tech, Gene Manipulation, Neural Interface, Sleep Deprivation
  - Dice pool configuration and computation
  - Roll button using the same free/spare time roll allocation
  - Live Testing support
- FTL assignments store metadata (dicePoolConfig, fringe effects, NPCs) via new metadata JSONField

## v0.10.41
- Project Roll System: GM allocates free rolls and spare time rolls per player or globally
  - Free rolls spent first; spare time rolls cost 1 mental load
  - Players can only roll on their own projects
  - WoD 2.0 dice: d10 pool, 8+ success, 10s explode, completion score updated
  - Roll allocation panel in Core Stats section
  - Roll button on each project card shows roll type and remaining count
- Linked NPC pulling strings now give XP-based bonus (+1 base + 1 per 15 XP) matching assigned NPC formula
- Pulling strings deduplicated with unique values — each Personal NPC selectable independently
- Dice pool auto-refreshes after every save (no more page reload needed)
- Overclocked Equipment destruction now costs 3-7 XP, logged in Base XP Log
- Projects assigned to bases via dropdown; NPCs unassigned on project completion

## v0.10.31
- Project Completion Effects: GM configures attribute changes (0-n) and integrity modifier per project
- COMPLETE PROJECT button applies effects and logs all stat changes
- LOG buttons on Integrity and Attributes sections showing historical stat changes from project completions
- Dice pool config panels stay open when making selections (no more closing on each change)
- Merits/pulling strings filtered to assigned character's own (with rating shown for merits)
- Pulling strings show linked NPC name and class (e.g. "Personal NPC — Dr. Chen (science)")
- Dice pool total always visible; hover shows color-coded breakdown tooltip
- NPC assignment to projects: each NPC gives +1 base + 1 per 15 XP to dice pool (one project at a time)
- Dossiers section: filter dropdown by agency (All, Players, Bifrost, etc.) with counts
- GM sees all NPC dossiers from all agencies, tagged with agency name
- Project Dice Pool: GM-configurable dice pool per project
  - Select 0-2 character attributes, 1 skill, 0-2 agency attributes
  - Choose applicable merits (+rating) and pulling strings (+1 each)
  - Workspace quality and mental load auto-applied
  - Fringe modifiers auto-included in pool total
  - Computed server-side with expandable breakdown display

## v0.10.27
- 6 new fringe effects for science class projects:
  - Black Market Tech (+2 dice, 15% tracker risk → agency linkage)
  - Gene Manipulation (+3 dice, 18% containment breach → GM narrative)
  - Neural Interface (+4 dice, 30% ML+2, 5% permanent -1 mental attribute)
  - Sleep Deprivation Marathon (+2 dice, 30% ML+1 to all team members)
  - Overclocked Equipment (+3 dice, 25% facility level loss at selected base)
  - Child Prodigy Recruitment (+1 permanent dice, 2 XP cost, 15% integrity loss, creates NPC dossier)
- All effects are one-time per project, stacking with existing fringe effects
- Child Prodigy NPCs tagged in dossier; can be assigned to other fringe projects for +2 dice (one at a time, reassignable)
- Overclocked Equipment destruction now costs 3-7 XP from agency, logged in Base XP Log
- Base XP Log displayed in Core Stats section
- Projects now have a BASE assignment dropdown (existing projects can remain unassigned)
- Fringe Science RULES button — toggleable manual explaining all fringe effects
